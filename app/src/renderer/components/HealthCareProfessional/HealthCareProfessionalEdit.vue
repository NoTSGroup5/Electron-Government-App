...
<script>
    ...
    export default {
        ...
        created(){
           Promise.all([httpOrganisationService.fetch(), httpHealthCareProfessionalService.getById(this.$route.params.bsn)]).then(result => {
                this.organisations = result[0];
                this.model = result[1];
           }).catch(() => alert('An error occurred while retrieving the organisations and health care professional.'));
        },
        methods: {
            ...
            validateForm(){
                this.$validator.validateAll().then(() => {
                    let id = this.model.bsn;
                    delete this.model.bsn;
                    httpHealthCareProfessionalService.update(id, this.model).then(() => {
                        this.$router.push({name: "healthCareProfessionalsOverview"});
                    }).catch(() => {
                        alert('An error occurred while updating the health care professional')
                    });
                }).catch();
            }
        }
